...
from ..misc import is_valid_hostname, is_valid_ipv4, is_valid_ipv6
...
class JobMbus:
    def __init__(self, engine: Engine, refresh_s: float = 30.0, n_threads: int = 40) -> None:
        ...
        # private
        self._workers_in_q: Queue[WorkerData] = Queue()
        self._workers_out_q: Queue[WorkerData] = Queue()
    ...
    def _populate_input_queue(self):
        with Session(self.engine) as session:
            for icmp in session.query(Icmp).filter(Icmp.icmp_inhibition == '0').all():
                ...
                self._workers_in_q.put(worker_data)
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                while True:
                    try:
                        worker_data = self._workers_out_q.get_nowait()
    ...
    def _worker_code(self):
        ...
        while True:
            # try/finally block for ensure task_done() is call for input queue
            try:
                # get node dict from in queue (thread add new key with thread_ prefix to it)
                worker_data = self._workers_in_q.get()
                # enforce security by checking hostname field
                hostname_valid = is_valid_ipv4(worker_data.hostname) or is_valid_ipv6(worker_data.hostname) or \
                    is_valid_hostname(worker_data.hostname)
                if not hostname_valid:
                    # warn user
                    logger.warning(f'worker is unable to process hostname "{worker_data.hostname}"')
                    worker_data.state = 'E'
                    self._workers_out_q.put(worker_data)
                    # process next one
                    continue
                # ping it
                args = f'/bin/ping -c 1 -W {worker_data.timeout} {worker_data.hostname}'.split()
                ...
                # ping return 0 if up
                worker_data.update = datetime.now()
                ...
                if ret_code == 0:
                    ...
                    if search:
                        try:
                            worker_data.rtt = int(round(float(search.group(2))))
                        except (AttributeError, ValueError):
                            worker_data.rtt = None
